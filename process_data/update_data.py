...
class Update_data():
    ...
    def process_data(self, actions):
        self.specials = {'add': [], 'replace': [], 'delete': []}
    ...
    def double_oper(self, action, action_list):
        ...
        for feature_index_tuple in action_list:
            ...
            if special_op == 0:
                ...
                self.specials['add'].append([feature1.values, new_fe])
            elif special_op == 1:
    ...
    def single_oper(self, action, action_list):
        ...
        for index, operation in action_list.items():
            ...
            if operation[1] == 0:
                ...
                self.specials['add'].append([ori_feature.values, new_fe])
            elif operation[1] == 1:
    ...
    def feature_cross(self, action, action_list):
        if action == "cross":
            for actions in action_list:
                ...
                new_fe = generate_cross_fe(ori_fe1.values, ori_fe2)
            ...
            for actions in action_list:
                ...
                if actions[2] == 0:
                    self.cross[name] = new_fe
                    self.specials['add'].append([ori_fe1.values, new_fe])
                elif actions[2] == 1:
                    self.cross[ori_fe1.name] = new_fe
                    self.cross = self.cross.rename(columns={ori_fe1.name: name})
